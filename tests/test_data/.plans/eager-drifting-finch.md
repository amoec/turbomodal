...
**b. Replace "Install with ML Support" section (lines 37-56)** — Change `pip install turbomodal` to `pip install -e ".[ml]"` (from-source). Keep the dependency table.

**c. Replace "Install Everything" section (lines 59-73)** — Change `pip install "turbomodal[ml,dev]"` to `pip install -e ".[dev]"` (from-source).
