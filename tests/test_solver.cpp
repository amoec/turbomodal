...
TEST(Solver, Known4x4System) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        EXPECT_NEAR(result.frequencies(i), f_expected, 1e-6)
            << "Frequency " << i << ": " << result.frequencies(i) << " vs expected " << f_expected;
    }
}
...
TEST(Solver, ModeShapesOrthogonalSmallSystem) {
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = i + 1; j < 3; j++) {
            double dot = std::abs(result.mode_shapes.col(i).dot(result.mode_shapes.col(j)));
            EXPECT_NEAR(dot, 0.0, 1e-6) << "Modes " << i << " and " << j << " not orthogonal";
        }
    }
}
...
TEST(Solver, Known5x5System) {
    ...
    for (int i = 0; i < n; i++) {
        K_dense(i, i) = 2.0;
        if (i > 0) K_dense(i, i - 1) = -1.0;
        if (i < n - 1) K_dense(i, i + 1) = -1.0;
    }
...
TEST(Solver, EliminateDofsDimensions) {
    ...
    auto [K_red, M_red, free_map] = ModalSolver::eliminate_dofs(K, M, constrained);
...
TEST(Solver, ExpandModeShapes) {
    ...
    Eigen::MatrixXcd full = ModalSolver::expand_mode_shapes(reduced, full_ndof, free_map);
...
TEST(Solver, SolveWithConstrainedDofs) {
    ...
    // Expand mode shapes
    Eigen::MatrixXcd full_modes = ModalSolver::expand_mode_shapes(
        result.mode_shapes, n, free_map);
...
TEST(Solver, SingleElementConstrainedVibration) {
    ...
    // Clamp the base face (nodes 0,1,2 and mid-edge nodes 4,5,6) — 6 nodes × 3 DOF = 18 constrained
    // This leaves only nodes 3,7,8,9 free (12 DOFs), so request up to 11 eigenvalues
...
TEST(Solver, ComplexHermitianRealCase) {
    ...
    // Use 5x5 system so real solver can request 4, and doubled (10x10) has plenty of room
    int n = 5;
    Eigen::MatrixXd K_dense = Eigen::MatrixXd::Zero(n, n);
    for (int i = 0; i < n; i++) {
        K_dense(i, i) = 2.0;
    ...
    auto [result_complex, status_complex] = solver.solve_complex_hermitian(K_complex, M_complex, config);

    ASSERT_TRUE(status_real.converged) << status_real.message;
    ASSERT_TRUE(status_complex.converged) << status_complex.message;

    int num_compare = std::min(result_real.frequencies.size(), result_complex.frequencies.size());
