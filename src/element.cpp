...
//
// Barycentric coordinates: L1 = 1 - xi - eta - zeta, L2 = xi, L3 = eta, L4 =
// zeta

Vector10d TET10Element::shape_functions(double xi, double eta,
                                        double zeta) const {
...
Matrix10x3d TET10Element::shape_derivatives(double xi, double eta,
                                            double zeta) const {
  ...
  // Node 0: N0 = L1*(2*L1 - 1)
  // dN0/dxi = dL1/dxi * (2*L1-1) + L1 * 2 * dL1/dxi = (-1)*(2*L1-1) + L1*2*(-1)
  // = -(2*L1-1) - 2*L1 = 1 - 4*L1
...
Eigen::Matrix3d TET10Element::jacobian(double xi, double eta,
                                       double zeta) const {
...
Matrix6x30d TET10Element::B_matrix(double xi, double eta, double zeta) const {
  ...
  // Chain rule: ∂f/∂x = J^{-T} * ∂f/∂ξ (column), transposed as rows: dNdx =
  // dNdxi * J^{-T}
