...
class _RemovedTestClass:
    ...
    def mesh_and_sensors(self, wedge_mesh_path):
        ...
        btt_cfg = _RemovedClass.default_btt_array(
            num_probes=4, casing_radius=0.15,
            axial_positions=[0.005], sample_rate=100_000.0, duration=0.01,
        )
        btt_vsa = _RemovedClass(mesh, btt_cfg)
        return mesh, btt_vsa

    def test_returns_plotter(self, mesh_and_sensors):
                mesh, vsa = mesh_and_sensors
        plotter = _removed_func(mesh, vsa, off_screen=True)
        assert plotter is not None
        plotter.close()

    def test_no_mesh_background(self, mesh_and_sensors):
                mesh, vsa = mesh_and_sensors
        plotter = _removed_func(mesh, vsa, show_mesh=False, off_screen=True)
        assert plotter is not None
        plotter.close()

    def test_no_directions(self, mesh_and_sensors):
                mesh, vsa = mesh_and_sensors
        plotter = _removed_func(mesh, vsa, show_directions=False, off_screen=True)
        assert plotter is not None
        plotter.close()

    def test_full_annulus(self, mesh_and_sensors):
                mesh, vsa = mesh_and_sensors
        plotter = _removed_func(mesh, vsa, full_annulus=True, off_screen=True)
        assert plotter is not None
        plotter.close()

    def test_uniform_color(self, mesh_and_sensors):
                mesh, vsa = mesh_and_sensors
        plotter = _removed_func(mesh, vsa, color_by="uniform", off_screen=True)
        assert plotter is not None
        plotter.close()

    def test_mixed_sensor_types(self, wedge_mesh_path):
        from turbomodal._core import Mesh
                    _RemovedClass, _RemovedClass, _RemovedEnum, _RemovedClass,
        )
        ...
        cfg = _RemovedClass(sensors=[
            _RemovedClass(
                sensor_type=_RemovedEnum.BTT_PROBE,
                position=np.array([0.15, 0.0, 0.005]),
                direction=np.array([-1.0, 0.0, 0.0]),
                label="BTT_0",
            ),
            _RemovedClass(
                sensor_type=_RemovedEnum.DISPLACEMENT,
...
class _RemovedTestClass:
    ...
    def test_returns_figure(self, signal_setup):
        ...
        fig = _removed_func(vsa, results, 3000.0)
    ...
    def test_filter_by_nd(self, signal_setup):
        ...
        mesh, vsa, results = signal_setup
        fig = _removed_func(vsa, results, 3000.0, nd=1)
        axes = fig.get_axes()
    ...
    def test_filter_by_nd_and_nc(self, signal_setup):
        ...
        mesh, vsa, results = signal_setup
        fig = _removed_func(vsa, results, 3000.0, mesh=mesh, nd=1, nc=0)
    ...
    def test_sensor_subset(self, signal_setup):
        ...
        mesh, vsa, results = signal_setup
        fig = _removed_func(vsa, results, 3000.0, sensors=[0, 2])
        axes = fig.get_axes()
        assert len(axes) == 2
        plt.close(fig)
