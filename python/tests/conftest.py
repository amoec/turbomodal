...
@pytest.fixture(scope="session")
def blade_mesh_path(test_data_dir):
    """Path to the blade sector mesh file (blisk with hub + blade)."""
    p = test_data_dir / "blade_sector.msh"
    if not p.exists():
        pytest.skip(f"Test mesh not found: {p}")
    return str(p)
...
@pytest.fixture(scope="session")
def solved_blade(blade_mesh_path):
    """Solve blade sector once for entire test session."""
    from turbomodal._core import Mesh, Material, CyclicSymmetrySolver

    mesh = Mesh()
    mesh.num_sectors = 24
    mesh.load_from_gmsh(blade_mesh_path)
    mesh.identify_cyclic_boundaries()
    mesh.match_boundary_nodes()
    mat = Material(200e9, 0.3, 7800)
    solver = CyclicSymmetrySolver(mesh, mat)
    results = solver.solve_at_rpm(0, 10)
    return mesh, mat, results
