...
def _removed_func(
    ...
    color_by : ``"type"`` colours sensors by _RemovedEnum, ``"uniform"`` uses a single colour
    ...
    _type_colors = {
        _RemovedEnum.BTT_PROBE: "#2166ac",             # blue
        _RemovedEnum.DISPLACEMENT: "#9467bd",           # purple
        _RemovedEnum.STRAIN_GAUGE: "#2ca02c",           # green
        _RemovedEnum.CASING_ACCELEROMETER: "#ff7f0e",   # orange
    }
    ...
    if color_by == "type":
        groups: dict[_RemovedEnum, list[int]] = {}
        for i, s in enumerate(sensors):
            groups.setdefault(s.sensor_type, []).append(i)
    else:
        groups = {_RemovedEnum.BTT_PROBE: list(range(n_sensors))}
        _type_colors[_RemovedEnum.BTT_PROBE] = "#d62728"  # uniform red
...
def _removed_func(
    ...
    :func:`~turbomodal (internal)` for a
    ...
    modes : Mode index/indices within each ND to include (0-based,
        ...
        and *nc* filtering.
    config : :class:`~turbomodal (internal)`.
