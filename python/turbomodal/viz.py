...
def plot_zzenf(
    ...
    if _need_crossings:
        try:
            from turbomodal_ext.signal_gen import find_resonance_crossings
        except ImportError:
            import warnings
            warnings.warn(
                "crossing_markers requires turbomodal-ext; "
                "install it for crossing support",
                stacklevel=2,
            )
            _need_crossings = False
    if _need_crossings:
